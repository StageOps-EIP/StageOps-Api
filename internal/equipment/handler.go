...
func (h *Handler) List(c *fiber.Ctx) error {
	...
	if err != nil {
		return respondError(c, fiber.StatusInternalServerError, "INTERNAL_ERROR", "Erreur lors de la récupération des équipements.")
	}
...
func (h *Handler) Get(c *fiber.Ctx) error {
	...
	if err != nil {
		...
		return respondError(c, fiber.StatusInternalServerError, "INTERNAL_ERROR", "Erreur lors de la récupération de l'équipement.")
	}
...
func mapError(c *fiber.Ctx, err error) error {
	...
	if errors.Is(err, ErrInvalidCategory) {
		return respondError(c, fiber.StatusBadRequest, "VALIDATION_ERROR", "Catégorie invalide. Valeurs acceptées : sound, light, video, set, safety, rigging.")
	}
